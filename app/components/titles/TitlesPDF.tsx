        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <Text style={styles.subtitle}>
                    {filtersText ? filtersText : `Total: ${titles.length}`}
                </Text>
                ...
                {titles.map((t, index) => (
                    <View key={t._id} style={styles.item}>
                        ...
                        <Text style={styles.line}>
                            Rating: {typeof t.rating === "number" ? t.rating : "-"}
                        </Text>
                        <Text style={styles.line}>
                            Plataforma: {t.platform ? t.platform : "-"}
                        </Text>

                        {/* Notas según estado */}
                        {t.notePre ? (
                            <Text style={styles.note}>Descripción previa: {t.notePre}</Text>
                        ) : null}

                        {t.notePost ? (
                            <Text style={styles.note}>Opinión: {t.notePost}</Text>
                        ) : null}
                    </View>
                ))}
            </Page>
