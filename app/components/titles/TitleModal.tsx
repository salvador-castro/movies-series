...
function Stars10({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
...
export default function TitleModal({
  ...
  const [f, setF] = useState<FormState>(() => ({
    ...
    rating:
      typeof initial?.rating === "number" && initial.rating >= 1 && initial.rating <= 10
        ? initial.rating
        : 8,
    imdb: typeof (initial as any)?.imdbId === "string" ? (initial as any).imdbId : "",
    filmaffinity:
      typeof (initial as any)?.filmaffinityId === "string" ? (initial as any).filmaffinityId : "",
  ...
  useEffect(() => {
    setF((prev) => ({
      ...prev,
      genres: prev.genres.filter((g) => (genreOptions as string[]).includes(g)),
    }));
  ...
  function validate(): string | null {
    ...
    if (f.filmaffinity.trim() && faN === null)
      return "FilmAffinity debe ser un número entre 0 y 10 (ej: 8.0).";

    if (f.imdb.trim() && f.filmaffinity.trim())
      return "Completá IMDb o FilmAffinity, pero no ambos.";
    ...
    <div className={styles.overlay} onMouseDown={onClose}>
      <div className={styles.modal} onMouseDown={(e) => e.stopPropagation()}>
        <div className={styles.modalHeader}>
          <h3 className={styles.modalTitle}>{mode === "create" ? "Agregar" : "Editar"}</h3>
          <button className={styles.modalClose} onClick={onClose}>
            Cerrar
          </button>
        </div>

        {error ? <div className={[styles.toast, styles.toastErr].join(" ")}>{error}</div> : null}
        ...
        <form onSubmit={submit} style={{ display: "grid", gap: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className={styles.field}>
              ...
              <select
                ...
                value={f.kind}
                onChange={(e) => setF((p) => ({ ...p, kind: e.target.value as TitleKind }))}
              >
            ...
            <div className={styles.field}>
              ...
              <select
                className={styles.select}
                value={f.state}
                onChange={(e) => setF((p) => ({ ...p, state: e.target.value as TitleState }))}
              >
          ...
          <div className={styles.field}>
            ...
            <GenreMultiSelect
              ...
              value={f.genres}
              onChange={(genres) => setF((p) => ({ ...p, genres }))}
            />
          </div>
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                onChange={(e) =>
                  setF((p) => ({
                    ...p,
                    imdb: normalizeDecimalInput(e.target.value),
                    filmaffinity: e.target.value.trim() ? "" : p.filmaffinity,
                  }))
                }
                placeholder="Ej: 9.5"
              />
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                onChange={(e) =>
                  setF((p) => ({
                    ...p,
                    filmaffinity: normalizeDecimalInput(e.target.value),
                    imdb: e.target.value.trim() ? "" : p.imdb,
                  }))
                }
                placeholder="Ej: 8"
              />
          ...
          <div className={styles.field}>
            ...
            <textarea
              ...
              value={f.notePre}
              onChange={(e) => setF((p) => ({ ...p, notePre: e.target.value }))}
            />
          </div>
          ...
          {f.state === "vista" ? (
            <div className={styles.field}>
              ...
              <textarea
                className={styles.textarea}
                value={f.notePost}
                onChange={(e) => setF((p) => ({ ...p, notePost: e.target.value }))}
              />
            </div>
          ) : null}
