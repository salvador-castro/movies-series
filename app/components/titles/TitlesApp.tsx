...
export default function TitlesApp() {
  ...
  // ✅ filtro por plataforma (backend)
  const [platformFilter, setPlatformFilter] = useState<string>("all");

  // ✅ paginación
  const [page, setPage] = useState(1);

  // ✅ plataformas disponibles (derivadas de lo que vino del backend)
  ...
  const qs = useMemo(() => {
    ...
    const s = sp.toString();
    return s ? `?${s}` : "";
  }, [q, kind, platformFilter]);

  // ✅ tab es filtro front (sobre lo que ya vino filtrado por backend)
  ...
  return (
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        {toast ? (
          <div
            className={[styles.toast, toast.type === "ok" ? styles.toastOk : styles.toastErr].join(
              " "
            )}
          >
            {toast.msg}
        ...
        <div className={styles.tabs}>
          <button
            className={[styles.tab, tab === "all" ? styles.tabActive : ""].join(" ")}
            onClick={() => setTab("all")}
          >
            Todos <span className={styles.tabBadge}>{counts.total}</span>
          </button>
          <button
            className={[styles.tab, tab === "para_ver" ? styles.tabActive : ""].join(" ")}
            onClick={() => setTab("para_ver")}
          >
            Para ver <span className={styles.tabBadge}>{counts.para_ver}</span>
          </button>
          <button
            className={[styles.tab, tab === "viendo" ? styles.tabActive : ""].join(" ")}
            onClick={() => setTab("viendo")}
          >
            Viendo <span className={styles.tabBadge}>{counts.viendo}</span>
          </button>
          <button
            className={[styles.tab, tab === "vista" ? styles.tabActive : ""].join(" ")}
